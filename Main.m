...
T_init = 293.15;    %K
rho_liq = py.CoolProp.CoolProp.PropsSI('D','T',T_init,'Q', 0,'NitrousOxide');
rho_vap = py.CoolProp.CoolProp.PropsSI('D','T',T_init,'Q', 1,'NitrousOxide');
u_liq = py.CoolProp.CoolProp.PropsSI('U','T',T_init,'Q', 0,'NitrousOxide');
u_vap = py.CoolProp.CoolProp.PropsSI('U','T',T_init,'Q', 1,'NitrousOxide');
...
T_wall_init=T_init;                                             %initial temperature of the tank wall (K)
...
for i=1:length(T_tank)
    ...
    if ~ismember(i,empty_tank)
        ...
        P_tank(i) = polyval(opts.Psat_NO2_polynom,T_tank(i))*10^5;
